...
edTest(
  `Should change the page style when the light mode/dark mode toggle is clicked. The toggle button should apply a class of "light" or "dark" to the body, depending on the toggle state`,
  () => {
...
edTest(
  'Should change the "--circle-color" attribute when the light mode/dark mode toggle is clicked. This property can be modified by calling document.documentElement.style.setProperty(<key>, <value>)',
  () => {
    const toggle = document.querySelector('#toggle');
    ...
    if(toggle){
      ...
      return {
        ...
        passed: false,
        feedback:
          'There is no element with an id of toggle',
        expand_feedback: true,
      };
    }
